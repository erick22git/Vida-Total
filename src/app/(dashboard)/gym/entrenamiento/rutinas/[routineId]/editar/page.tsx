...
export default function EditRoutinePage({
  ...
  function replaceActive(newId: string) {
    setDraft((d) => d.map((ex, i) => (i === activeIndex ? { ...ex, exerciseId: newId } : ex)));
      ...
      {activeExercise && active && (
        <>
          ...
          <div className="flex gap-2">
            <ActionButton icon={Repeat} label="Reemplazar" onClick={() => setReplacePickerOpen(true)} />
          ...
          <GlassInput
            ...
            value={active.nota ?? ""}
            onChange={(e) =>
              setDraft((d) => d.map((ex, i) => (i === activeIndex ? { ...ex, nota: e.target.value } : ex)))
            }
          />
          ...
          <RoutineSetTable
            ...
            onChange={(sets) => setDraft((d) => d.map((ex, i) => (i === activeIndex ? { ...ex, sets } : ex)))}
