...
import { CalendarClock, CalendarPlus, ChevronLeft, ChevronRight, Copy, Inbox, LayoutGrid, List, Plus, RotateCcw, Settings, Sparkles, Trash2 } from "lucide-react";
import { occurrencesOn } from "@/lib/agenda/recurrence";
import { pendingItems } from "@/lib/agenda/replan";
...
import { ReplanScreen } from "./replan-screen";
...
type Tab = "inbox" | "day" | "ai" | "settings";
...
export function AgendaScreen() {
  ...
  const [dayMenu, setDayMenu] = useState<null | "menu" | "copy" | "clear">(null);
  ...
  const overdue = useMemo(() => pendingItems(tasks, now), [tasks, now]);
  ...
  const title =
    tab === "inbox" ? "Bandeja" : tab === "ai" ? "IA" : tab === "settings" ? "Ajustes" : expanded ? format(selected, "d MMM yyyy", { locale: es }).replace(/\./g, "") : format(selected, "MMMM 'de' yyyy", { locale: es });
    ...
    <div className="fixed inset-0 z-[45] text-white bg-black overflow-hidden">
      <div className="relative mx-auto h-full w-full max-w-[430px] overflow-hidden flex flex-col" style={{ background: "#000" }}>
        ...
        <header className="shrink-0 pt-[max(env(safe-area-inset-top),14px)]">
                      ...
                      {dots.map((t, k) => {
                        return (
                          <span key={t.id} className="flex items-center justify-center w-[18px] h-[18px] rounded-full" style={{ background: t.done ? "#555" : "#fff", color: "#111", marginLeft: k ? -6 : 0 }}>
                            <AgendaIcon k={t.icon} size={10} strokeWidth={3} />
                          </span>
                        );
                      })}
                    </span>
          ...
          {tab === "day" && overdue.length > 0 && date === today && (
            <button onClick={() => setReplan({ scope: null })} data-testid="replan-banner" className="mx-4 mb-2 flex items-center justify-center gap-2 h-9 px-4 rounded-full text-[14px] font-extrabold cursor-pointer" style={{ background: "#2b2b2e" }}>
              <RotateCcw size={15} /> {overdue.length} {overdue.length === 1 ? "pendiente" : "pendientes"} · Revisar
            </button>
          )}
        </header>
          ...
          <div className="flex-1 overflow-y-auto px-5 pt-4 pb-40">
            {inbox.length === 0 ? (
              <div className="flex flex-col items-center gap-3 mt-24 text-center" style={{ color: "rgba(255,255,255,0.5)" }}>
                ...
                <p className="text-[15px] font-semibold max-w-[240px]">Aquí van las tareas sin día ni hora. Toca el reloj de una tarea para programarla y pasará a la línea de tiempo.</p>
              </div>
            ) : (
              inbox.map((t) => (
                <div key={t.id} role="button" aria-label={`Abrir ${t.title}`} onClick={() => setEditor({ mode: "edit", id: t.id, date: toISODate(new Date()) })} className="flex items-center gap-4 mb-4 cursor-pointer">
                  <TaskNode icon={t.icon} color={t.color} width={50} height={50} iconSize={22} done={t.done} source={t.source} label={t.title} />
                  ...
                  <button aria-label={`Programar ${t.title}`} onClick={(e) => { e.stopPropagation(); setScheduleId(t.id); }} className="w-10 h-10 rounded-full flex items-center justify-center cursor-pointer shrink-0" style={{ background: "#2b2b2e" }}>
        ...
        {tab === "ai" && (
          <AiView
            onEdit={(prefill) => setEditor({ mode: "create", date: prefill.date ?? date, startMin: prefill.startMin ?? null, prefill })}
            onOpenDay={(d) => { setDate(d); setSelectedId(null); setTab("day"); }}
          />
        )}
        ...
        {replan && <ReplanScreen scopeDate={replan.scope} onClose={() => setReplan(null)} />}
        <ScheduleSheet key={scheduleId ?? "none"} open={!!scheduleId} onClose={() => setScheduleId(null)} task={inbox.find((t) => t.id === scheduleId) ?? null} />
        ...
        {dayMenu === "menu" && (
          <div className="absolute inset-0 z-[55]" onClick={() => setDayMenu(null)}>
            <div className="absolute left-3 top-[112px] w-[250px] rounded-[28px] p-2" style={{ background: "rgba(28,28,30,0.97)", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 16px 50px rgba(0,0,0,0.7)" }} onClick={(e) => e.stopPropagation()} role="menu" aria-label="Opciones del día">
              ...
              {overdue.length > 0 && (
                <MenuRow icon={<RotateCcw size={22} />} onClick={() => { setDayMenu(null); setReplan({ scope: null }); }}>Revisar tareas pendientes ({overdue.length})</MenuRow>
              )}
