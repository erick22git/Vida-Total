...
import { Bell, BellOff, Calendar, Check, Clock, Copy, Ellipsis, Inbox, Palette, Repeat, Target, Trash2, X } from "lucide-react";
import { iconForTitle } from "@/lib/agenda/icons";
import { emptyDraft, useAgendaStore } from "@/lib/agenda/store";
import { durationLabel, isInProgress, minutesLeft, nowMinutes, taskProgress, timeRange, toISODate } from "@/lib/agenda/time";
...
export type EditorTarget = { mode: "create"; date: string | null; startMin: number | null } | { mode: "edit"; id: string };
...
function dateHint(iso: string | null): string {
  ...
  return iso === tomorrow ? "Mañana" : "";
}

function defaultStart(date: string | null, step: number): number {
  const now = new Date();
  if (date === toISODate(now)) return Math.min(23 * 60, Math.ceil((nowMinutes(now) + 1) / step) * step);
  return 9 * 60;
}
...
export function TaskEditor({ target, onClose, onFocus }: { target: EditorTarget; onClose: (createdId?: string) => void; onFocus?: (id: string) => void }) {
  ...
  const stored = isEdit ? tasks.find((t) => t.id === target.id) : undefined;
  const [local, setLocal] = useState<AgendaTaskDraft>(() =>
    target.mode === "create" ? emptyDraft(target.date, target.date === null ? null : (target.startMin ?? defaultStart(target.date, useAgendaStore.getState().timeStep))) : emptyDraft(null, null),
  );
  // En edición se lee siempre del store (una sola fuente de verdad); en creación, del borrador local.
  ...
  const [sheet, setSheet] = useState<null | "date" | "duration" | "color" | "time" | "timeMenu" | "more">(null);
  const [subDraft, setSubDraft] = useState("");
  const iconTouched = useRef(isEdit);
  const [hint, setHint] = useState("");

  const start = draft.startMin ?? defaultStart(draft.date, step);
  const inProgress = !!stored && isInProgress(stored, now);
  const compactHeader = isEdit && !inProgress;
  ...
  function finishCreate() {
    ...
    const id = addTask({ ...draft, title: draft.title.trim() || "Nueva tarea", startMin: draft.date === null || draft.allDay ? null : start });
  ...
  const remaining = stored && inProgress ? minutesLeft(stored, now) : null;
  ...
  return (
    <div className="absolute inset-0 z-[60] flex flex-col" style={{ background: "#000" }}>
      <div className="relative flex-1 flex flex-col mt-[max(env(safe-area-inset-top),20px)] overflow-hidden" style={{ background: SURFACE, borderTopLeftRadius: 34, borderTopRightRadius: 34 }}>
        {/* ---------- Cabecera clara ---------- */}
        <div className="relative shrink-0" style={{ background: LIGHT, color: "#111", borderTopLeftRadius: 34, borderTopRightRadius: 34, minHeight: compactHeader ? 78 : stage === 1 ? 150 : 236, transition: "min-height 240ms ease" }}>
          ...
          {compactHeader ? (
            <>
              <input
                ...
                aria-label="Título"
                className="absolute inset-x-0 top-[22px] mx-auto w-[60%] bg-transparent text-center text-[24px] font-extrabold outline-none"
              />
            ...
            <div className="flex items-start gap-4 pl-7 pr-5 pt-[64px] pb-5">
              <div className="relative shrink-0">
                <TaskNode
                  ...
                  progress={stored ? taskProgress(stored, now) : 0}
              ...
              <button
                aria-label={draft.done ? "Marcar como pendiente" : "Completar"}
                onClick={() => {
                  haptic("light");
                  if (isEdit && stored) toggleDone(stored.id);
                  else patch({ done: !draft.done });
                }}
                className="shrink-0 mt-7 flex items-center justify-center w-[26px] h-[26px] rounded-full cursor-pointer"
                style={{ border: "2.5px solid #111", background: draft.done ? "#111" : "transparent", color: LIGHT }}
              >
                {draft.done && <Check size={16} strokeWidth={4} />}
              </button>
            </div>
          )}
        </div>
          ...
          {stage === 1 ? (
            ...
            <>
              <div className="rounded-[26px] overflow-hidden" style={{ background: CARD }}>
                <Row icon={<Calendar size={24} />} onClick={() => setSheet("date")} right={dateHint(draft.date)}>{dateText(draft.date)}</Row>
                {draft.date !== null && (
              ...
              <button onClick={() => setHint("Las tareas que se repiten llegan pronto.")} className="flex items-center gap-3 h-14 px-5 mt-3 rounded-full cursor-pointer text-[19px] font-bold" style={{ background: CARD, color: "rgba(255,255,255,0.7)" }}>
                <Repeat size={22} /> Repetir
              </button>
              {hint && <p className="text-[13px] mt-2 px-2" style={{ color: "rgba(255,255,255,0.5)" }}>{hint}</p>}
        ...
        {(stage === 1 || !isEdit) && (
          <div className="absolute left-4 right-4 bottom-4 pb-[env(safe-area-inset-bottom)]">
            <button onClick={() => (stage === 1 ? setStage(2) : finishCreate())} className="w-full h-14 rounded-full text-[21px] font-extrabold cursor-pointer active:scale-[0.98] transition-transform" style={{ background: "#f4f4f5", color: "#111" }}>
      ...
      {sheet === "more" && stored && (
        <div className="absolute inset-0 z-[75]" onClick={() => setSheet(null)}>
          <div className="absolute right-4 top-[92px] w-[230px] rounded-[26px] p-2" style={{ background: "rgba(38,38,41,0.97)", boxShadow: "0 16px 50px rgba(0,0,0,0.6)" }} onClick={(e) => e.stopPropagation()}>
            <button className="flex items-center gap-3 w-full px-3 py-3 rounded-xl text-[17px] font-semibold cursor-pointer" onClick={() => { addTask({ ...stored, title: `${stored.title} (copia)`, done: false }); setSheet(null); onClose(); }}>
      ...
      <DatePickerSheet open={sheet === "date"} onClose={() => setSheet(null)} value={draft.date} allowNone onPick={(d) => patch(d === null ? { date: null, startMin: null } : { date: d, startMin: draft.startMin ?? defaultStart(d, step) })} />
      ...
      <TimeMoreMenu
        ...
        onInbox={() => patch({ date: null, startMin: null, allDay: false })}
      />
      <ColorIconSheet
        open={sheet === "color"}
